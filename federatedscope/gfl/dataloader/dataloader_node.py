...
def load_nodelevel_dataset(config=None):
    ...
    if name in ["cora", "citeseer", "pubmed"]:
        ...
        global_dataset = DBLPNew(path,
                                 ...
                                 splits=config.data.splits,
                                 **transforms_funcs)
    elif name == "ms_academic":
        from federatedscope.gfl.dataset.ms_academic import MSAcademic
        dataset = MSAcademic(path,
                          splits=config.data.splits,
                          **transforms_funcs)
        dataset = splitter(dataset[0])
        global_dataset = MSAcademic(path,
                                 splits=config.data.splits,
                                 **transforms_funcs)
    ...
    print("dataset = [ds for ds in dataset]")
