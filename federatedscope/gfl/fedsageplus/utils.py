...
class HideGraph(BaseTransform):
    ...
    :rtype:
        nx.Graph
    """
    def __init__(self, hidden_portion=0.5, num_pred=5,info_path=None):
    ...
    def __call__(self, data):
        ...
        if self.info_path is not None:
            self.info['impaired_feat']=data.x.numpy()[remaining_nodes]
            import pickle
...
def FillGraph(impaired_data, original_data, pred_missing, pred_feats,
    ...
    if info_path is not None:
        ...
        with open(info_path, 'rb') as f:
            x = pickle.load(f)
            info_dict={k:v for k,v in x.items()}
            info_dict['mend_feat']=new_features
...
@torch.no_grad()
def GraphMender(model, impaired_data, original_data,info_path=None):
    ...
    return FillGraph(impaired_data, original_data, pred_missing, pred_feats,
                     global_cfg.fedsageplus.num_pred,info_path)
