            ...
            try:
                ...
                if isinstance(batch, Data):
                    if isinstance(ctx.model,Classifier_F):
                        x, x_emb,edge_index = batch.x,batch.mend_emb, batch.edge_index
                    else:
                        x, edge_index = batch.x, batch.edge_index
                from fvcore.nn import FlopCountAnalysis
                if isinstance(ctx.model,Classifier_F):
                    flops_one_batch = FlopCountAnalysis(ctx.model,
                                                    (x,x_emb, edge_index)).total()
                else:
                    flops_one_batch = FlopCountAnalysis(ctx.model,
                                                        (x,edge_index)).total()
            ...
            if isinstance(ctx.get("{}_loader".format(ctx.cur_split)),
                ...
                if isinstance(ctx.model,Classifier_F):
                    if "mend_emb" not in ctx.data['data']:
                        ctx.data['data'].mend_emb = torch.zeros(
                            (len(ctx.data['data'].x),ctx.model.emb_len)).to(device)
                    ctx.data['data'].mend_emb = ctx.data['data'].mend_emb.to(ctx.device)
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        if ctx.cur_split == 'train':
            ...
            if self.is_NeighborSampler:
                ...
                if isinstance(ctx.model,Classifier_F):
                    if "mend_emb" not in ctx.data['data']:
                        mend_emb=torch.zeros((len(ctx.data['data'].x),ctx.model.emb_len)).to(device)
                    else:
                        mend_emb=ctx.data['data'].mend_emb
                    pred = ctx.model((ctx.data['data'].x[n_id],mend_emb[n_id]),
                                     adjs=adjs)
                else:
                ...
                if isinstance(ctx.model,Classifier_F):
                    if "mend_emb" not in batch:
                        mend_emb=torch.zeros((len(batch.x),ctx.model.emb_len)).to(device)
                    else:
                        mend_emb=batch.mend_emb
                    pred = ctx.model(batch.x,mend_emb, batch.edge_index)[batch['{}_mask'.format(
                        ctx.cur_split)]]
                else:
