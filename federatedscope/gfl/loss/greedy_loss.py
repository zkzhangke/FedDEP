...
def GreedyLoss_Modify(pred_feats, true_feats, pred_missing, true_missing, num_pred):
    r"""Greedy loss is a loss function of cacluating the MSE loss for the feature.
    https://proceedings.neurips.cc//paper/2021/file/ \
    34adeb8e3242824038aa65460a47c29e-Paper.pdf
    Fedsageplus models from the "Subgraph Federated Learning with Missing
    ...
    if CUDA:
        ...
        pred_missing = pred_missing.cpu()
    pred_len = len(pred_feats)
    pred_feats=pred_feats.view(pred_len,num_pred,-1)
    loss = torch.zeros(pred_feats.shape[:2])
    if CUDA:
        loss = loss.to(device)
    ...
    for i in range(pred_len):
        if true_missing_np[i] > 0:
            if isinstance(true_feats[i][true_missing_np[i] - 1],
                          np.ndarray):
                true_feats_i=torch.tensor(
                    true_feats[i]).to(device)
            else:
                true_feats_i=true_feats[i].to(device)
            for pred_j in range(min(num_pred, pred_missing_np[i])):
                true_feats_tensor =  true_feats_i[true_missing_np[i] - 1]
                loss[i][pred_j] = F.mse_loss(
                    pred_feats[i][pred_j].unsqueeze(0).float(),
                    true_feats_tensor.unsqueeze(0).float())

                for true_k in range(min(num_pred, true_missing_np[i]-1)):
                    true_feats_tensor = true_feats_i[true_k]
