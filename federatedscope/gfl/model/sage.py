...
class Encoder(SAGE_Net):
    ...
    def forward(self, x, edge_index=None, edge_weight=None, adjs=None,device='cpu'):
        if isinstance(x, torch.Tensor):
            if edge_index is None:
                if self.num_layers==1:
                    edge_index, _, size=adjs
                    x_target = x[:size[1]]
                    x = self.convs[0].to(device)((x.to(device), x_target.to(device)), edge_index.to(device))
                    x = F.tanh(x)
                    x = F.dropout(x,p=self.dropout,training=self.training)
                else:
                    for i, (edge_index, _, size) in enumerate(adjs):
                        x_target = x[:size[1]]
                        x = self.convs[i].to(device)((x.to(device), x_target.to(device)), edge_index.to(device))
                        if i!=self.num_layers-1:
                            x = F.relu(x)
                            x = F.dropout(x,p=self.dropout,training=self.training)
                        else:
                ...
                for conv in self.convs[:-1]:
                    x = conv.to(device)(x.to(device), edge_index.to(device), edge_weight.to(device))
                    x = F.relu(x)
                    x = F.dropout(x, p=self.dropout, training=self.training)
                x = self.convs[-1].to(device)(x.to(device), edge_index.to(device), edge_weight.to(device))
                x = F.tanh(x)
                x = F.dropout(x, p=self.dropout, training=self.training)
            x = self.linear.to(device)(x)
            x = F.softmax(F.dropout(x,p=self.dropout, training=self.training))
    ...
    def forward_full(self, data):
        if isinstance(data, Data):
            if 'mend_emb' not in data:
                ...
                x_emb=torch.zeros((len(x_feat),self.emb_len)).to(x_feat.device)
            else:
    ...
    def inference(self, x_all, subgraph_loader, device):
        ...
        for i in range(self.num_layers):
            ...
            for batch_size, n_id, adj in subgraph_loader:
                ...
                x_feat = self.add_convs[i][0].to(device)((x_feat, x_target), edge_index)
