...
def get_prototypes(emb,K,batch_size=None,CUDA=None,ae_pretrained_epochs=None,ae_finetune_epochs=None,dec_epochs=None):

    emb_shape = len(emb[0])
    K=int(K)
    proto_idx = train_clustering(node_embs=emb,num_prototypes=K,
...
def get_emb(all_data,_cfg):
    ...
    encoder=Encoder(in_channels=len(all_data.x[0]),out_channels=_cfg.feddep.encoder.out_channels,
    ...
    for epoch in range(_cfg.feddep.encoder.epochs):
        ...
        for batch_size, n_id, adjs in dataloader['train']:
            ...
            loss = torch.nn.functional.cross_entropy(out.to(device), all_data.y[n_id[:batch_size]].to(device))
            loss.backward()
            optimizer.step()
            total_loss += float(loss)
            total_correct += int(out.argmax(dim=-1).cpu().eq(all_data.y[n_id[:batch_size]]).sum().cpu())
    ...
    encoder.eval()
    all_emb=encoder.get_enc(dataloader['data'].x, dataloader['test'], device).detach()
...
class HideGraph(BaseTransform):
    ...
    def get_prototypes(self,X,config):
        ...
        self.prototypes,self.proto_idx=get_prototypes(emb=emb,K=self.num_proto,
    ...
    def __call__(self, data,config=None):
        ...
        self.x_missing=torch.zeros((len(data.x),self.num_pred,len(self.emb[0])))
        ...
        data.index_orig=torch.tensor([i for i in range(data.num_nodes)],dtype=torch.long)
        ...
        for i in G.nodes:
            ...
            if len(ids_missing) > 0:
                if len(ids_missing) <= self.num_pred:
                    G.nodes[i]['x_missing'] = torch.tensor(np.vstack(
                        (self.emb[ids_missing],
                         np.zeros((self.num_pred - len(ids_missing),
                                   self.emb.shape[1])))))

                else:
                    G.nodes[i]['x_missing'] = torch.tensor(self.emb[
                                                               ids_missing[:self.num_pred]])
            else:
                G.nodes[i]['x_missing'] = torch.zeros((self.num_pred,self.emb.shape[1]))
...
def FillGraph(impaired_data, original_data, pred_missing, pred_feats, num_pred):
    ...
    for node in range(len(pred_missing)):
        ...
        if num_fill_node > 0:
            ...
            mend_emb[org_id][:num_fill_node] += pred_feats[node][:num_fill_node]
