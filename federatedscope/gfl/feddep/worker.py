...
class FedDEPClient(Client):
    def __init__(self,
                 ...
                 **kwargs):
        super(FedDEPClient,
              self).__init__(ID, server_id, state, config, data,
                             model,
                             device,strategy, *args, **kwargs)
        self.data = data


        self.hide_data,self.emb,self.x_missing= HideGraph(hidden_portion=self._cfg.feddep.hide_portion,
                                                              num_pred=self._cfg.feddep.num_pred,
                                                              num_proto=self._cfg.feddep.num_proto)(data=data,config=self._cfg)

        self.data.emb=self.emb
        self.device = device
        self.sage_batch_size = 64
        self.gen = LocalDGen(in_channels=data.x.shape[-1],
    ...
    def callback_funcs_for_local_pre_train(self, message: Message):
        ...
        self.comm_manager.send(
            Message(msg_type='dep_para',
                    ...
                    receiver=[sender],
                    state=self.state,
                    content=[dep_para, embedding, self.hide_data.num_missing]))
    ...
    def callback_funcs_for_dep_para(self, message: Message):
        ...
        dep_grad = self.trainer_feddep.cal_grad(self.x_missing,self.emb, dep_para, embedding)
        self.state = round
        self.comm_manager.send(
            Message(msg_type='gradient',
                    sender=self.ID,
    ...
    def callback_funcs_for_setup_fedsage(self, message: Message):
        round, sender, _ = message.state, message.sender, message.content
        self.filled_data = GraphMender(model=self.feddep,
