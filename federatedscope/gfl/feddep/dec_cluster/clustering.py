...
def train_clustering(node_embs,num_prototypes,batch_size,CUDA,ae_pretrained_epochs,ae_finetune_epochs,dec_epochs):
    ...
    ds_train = Node_Emb_Dateset(node_embs=node_embs,testing_mode=False)  # training dataset
    ...
    print("DEC stage.")
    model = DEC(cluster_number=num_prototypes, hidden_dimension=emb_len, encoder=autoencoder.encoder)
    if CUDA:
        model.cuda()
    ...
    predicted = predict(
        ds_train, model, 1024, silent=True, cuda=CUDA)
