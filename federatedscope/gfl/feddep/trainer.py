...
class LocalGenTrainer(NodeFullBatchTrainer):
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        loss_rec = self.criterion_rec(pred_embs=pred_emb,
...
class FedDEPTrainer(LocalGenTrainer):
    ...
    def cal_grad(self, x_missing,G_emb, model_para, embedding):
        ...
        global_target_emb = []
        for c_i in choice:
            choice_i = np.random.choice(len(x_missing[c_i]),self.cfg.feddep.num_pred)
        ...
        loss = (1.0 / self.cfg.federate.client_num * self.cfg.feddep.beta_n * loss_emb).requires_grad_()
        loss.backward()
        grads = {
            key: value.grad
            for key, value in self.ctx.model.named_parameters()
